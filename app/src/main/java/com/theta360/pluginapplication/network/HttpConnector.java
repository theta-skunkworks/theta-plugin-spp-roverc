...
public class HttpConnector {
    ...
    public static final String API_URL_INFO = "/osc/info";
    public static final String API_URL_STAT = "/osc/state";
    public static final String API_URL_CMD_EXEC = "/osc/commands/execute";
    public static final String API_URL_CMD_STAT = "/osc/commands/status";
    ...
    /**
     * OptionNameと設定値を指定しパラメータを設定します
     *
     * @param value 設定するOptionの値
     * @return errorMessage (nullまたはエラーメッセージ)
     */
    public String setOption(String optionName, String value) {
        HttpURLConnection postConnection = createHttpConnection("POST", "/osc/commands/execute");
        JSONObject input = new JSONObject();
        String responseData;
        String errorMessage = null;
    ...
    /**
     * OptionNameを指定しパラメータを取得します
     *
     * @return 取得したOptionの値またはエラーメッセージ
     */
    public String getOption(String optionName) {
        ...
        try {
            ...
            if (status.equals("done")) {
                JSONObject results = output.getJSONObject("results");
                JSONObject options = results.getJSONObject("options");
                optionValue = options.getString(optionName);
            } else if (status.equals("error")) {
                JSONObject errors = output.getJSONObject("error");
                optionValue = errors.getString("message");
            }
        } catch (IOException e) {
            e.printStackTrace();
        } catch (JSONException e) {
